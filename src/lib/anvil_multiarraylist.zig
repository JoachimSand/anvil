    ...
    return struct {
        ...
        pub const Elem = switch (@typeInfo(T)) {
            ...
            .Union => |u| struct {
                ...
                // Own additions
                pub const TagType = std.meta.FieldEnum(T);
                pub const Slice = struct { tags: []TagType, data: []Bare };
        ...
        pub const Slice = struct {
            ...
            pub const All = blk: {
                const field_infos = std.meta.fields(Field);
                var structFields: [field_infos.len]std.builtin.Type.StructField = undefined;
                var decls = [_]std.builtin.Type.Declaration{};
                for (field_infos, 0..) |field, i| {
                    structFields[i] = .{
                        .name = field.name ++ "",
                        .type = []FieldType(@field(Field, field.name)),
                        .default_value = null,
                        .is_comptime = false,
                        .alignment = @alignOf([]FieldType(@field(Field, field.name))) * field_infos.len,
                    };
                }
                break :blk @Type(.{
                    .Struct = .{
            ...
            pub fn all_items(self: Slice) All {
                var all: All = undefined;
                const field_infos = std.meta.fields(All);
                inline for (field_infos) |field| {
                    @field(all, field.name) = self.items(@field(Field, field.name));
                }
                return all;
            }

            pub fn all_items_slice(self: Slice, start: usize, stop: usize) All {
                var all: All = undefined;
                const field_infos = std.meta.fields(All);
                inline for (field_infos) |field| {
                    @field(all, field.name) = self.items(@field(Field, field.name))[start..stop];
                }
                return all;
            }
