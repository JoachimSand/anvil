...
// Subset of token types. Enumerates the possible characters the tokeniser
...
const Tokeniser = struct {
    ...
    fn next_token(tokeniser: *Tokeniser) !Token {
...
pub fn main() !void {
    ...
    var tokeniser = Tokeniser{ .buf = input.items, .cur_pos = 0, .cur_tok_start = 0 };
    while (true) {
